...
Para começar, vamos definir a função \text{succ}, que recebe um número $n$ e retorna $n+1$ pela adição de outra aplicação de $f$.
\begin{flalign*}
SUCC~&:=~\lambda n. \lambda f. \lambda x. f (n~f~x) &
\end{flalign*}
$(n~f~x)$ quer dizer que $f$ será aplicada $n$ vezes em $x$. Por exemplo: 
\begin{flalign*}
3 \text{ sqrt } 7 &= \text{ sqrt(sqrt(sqrt(7)))} &
\end{flalign*}
...
\begin{cases}
b, \text{ se } a = \text{ TRUE}\\
a, \text{ se } a = \text{ FALSE}
\end{cases}&
a \vee b =
\begin{cases}
\text{TRUE}, \text{ se } a = \text{ TRUE}\\
b, \text{ se } a = \text{ FALSE}
\end{cases}&
\end{flalign*}
Sabendo disso...
...
\begin{flalign*}
K &= \lambda x \lambda y.x & &&\text{Retorna uma função contante}\\
